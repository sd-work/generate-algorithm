...
class Structure:
    ...
    # 00. 部材の色分けを行う - Master of color code program -
    def color_code_timbers(self):

        # 01. edgeを主構造体とサブ構造体に振り分ける
        ...
        # 02. 主構造体を構成するedgeの冗長性を判定し、サーフェスの色分けを行う
        self.judge_redundancy_of_test_timber(main_structure_edges, "main")

        # 02. サブ構造体を構成するedgeの冗長性を判定し、サーフェスの色分けを行う
    ...
    # 02. edge(main, sub)の冗長性を判定し、色分けを行う
    def judge_redundancy_of_test_timber(self, structure_edges=None, edge_type=None):
        ...
        # サブ構造
            ...
            for sub_edge in test_edges:
                ...
                # master timberが接合点を持たない + 地面に設置している場合、その部材は赤色
                # 例えば、最初の1本目を生成した時など
                elif len(sub_edge.timber.joint_pts_nodes) < 1 and sub_edge.timber.is_generated_from_GL:
                    # About master timber
                    rs.ObjectColor(sub_edge.timber.surface_guid, [0, 0, 0])  # default color
                    ...
                    if len(test_edges) >= 2:
                        ...
                        sub_edge.split_timber.status = 3  # オレンジ
    ...
    # 02-2. 各edgeが保持するTimber instanceのTimber Surfaceの色情報を設定する
    def set_timber_color(self, test_edge, degree_of_stability):
        ...
        # test edgeが破壊されても、不安定な構造体が生成されないので、冗長性があると判定する
            ...
            # master structureが安定構造である場合
                ...
                # About split timber
                if test_edge.split_timber.surface_guid:
                    rs.ObjectColor(test_edge.split_timber.surface_guid, [157, 204, 255])  # 青色
                    test_edge.split_timber.status = 2  # 青色
                else:
                    pass

    # 03. 各エッジのlayer分け、色分け、分割を行う
    def set_edges_to_main_sub_layer(self, split_num=10):
        ...
        for master_edge in self.main_edges:
            ...
            # set user text
            rs.SetUserText(master_edge.edge_line_guid, "color", "blue")  # joint
            ...
            if master_edge.split_edges_guid:
                ...
                if master_edge.split_edges:
                    ...
                    for divided_edge in master_edge.divided_two_edges:
                        divided_edge_guid = scriptcontext.doc.Objects.AddCurve(divided_edge)
                        master_edge.divided_two_edges_guid.append(divided_edge_guid)
                ...
                for i, split_edge_guid in enumerate(master_edge.split_edges_guid):
                    ...
                    # set user text
                    rs.SetUserText(split_edge_guid, "color", "blue")  # joint

                # 02. split edge(master edge)
        ...
        for edges in self.sub_edges:
            ...
            if len(edges) >= 2:
                for master_edge in edges:
                    ...
                    # master edgeが赤色かどうかを判定
                    if master_edge.split_timber.status == 0:
                        rs.ObjectColor(master_edge.edge_line_guid, [223, 51, 78])  # 赤色

                        # set user text
                        rs.SetUserText(master_edge.edge_line_guid, "color", "red")  # joint
                    else:
                        rs.ObjectColor(master_edge.edge_line_guid, [255, 140, 0])  # orange

                        # set user text
                        rs.SetUserText(master_edge.edge_line_guid, "color", "orange")  # joint
                    ...
                    if master_edge.split_edges_guid:
                        ...
                        for i, split_edge_guid in enumerate(master_edge.split_edges_guid):
                            ...
                            # master edgeが赤色かどうかを判定
                            if master_edge.split_timber.status == 0:
                                rs.ObjectColor(split_edge_guid, [223, 51, 78])  # 赤色

                                # set user text
                                rs.SetUserText(split_edge_guid, "color", "red")  # joint
                            else:
                                rs.ObjectColor(split_edge_guid, [255, 140, 0])  # orange

                                # set user text
                                rs.SetUserText(split_edge_guid, "color", "orange")  # joint
            ...
            # 02. 片持ち
                ...
                for master_edge in edges:
                    ...
                    if master_edge.split_timber.status == 0:
                        ...
                        rs.ObjectColor(master_edge.edge_line_guid, [225, 225, 0])  # 黄色

                        # set user text
                        rs.SetUserText(master_edge.edge_line_guid, "color", "yellow")  # joint
                    ...
                    if master_edge.split_edges_guid:
                        ...
                        for i, split_edge_guid in enumerate(master_edge.split_edges_guid):
                            ...
                            if master_edge.split_timber.status == 0:
                                ...
                                rs.ObjectColor(split_edge_guid, [225, 225, 0])  # 黄色

                                # set user text
                                rs.SetUserText(split_edge_guid, "color", "yellow")  # joint
